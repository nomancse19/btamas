...
ob_start();
class ExporterClientController extends CI_Controller {

	/**
	 * All exporter Page for this controller.
    ...
    public function exporter_all_chalan(){
        $data['exporter_chalan_info']=$this->clientControllerModel->all_exporter_chalan_info_by_exporter_id($this->session->userdata('exporter_info_id'));
        $data['admin_main_content']=$this->load->view('Clientpage/exporter_all_chalan',$data,TRUE);
	    $this->load->view("Clientpage/client_master",$data);
    }


    public function exporter_vehicles_live_tracking($chalan_no){
        ...
        if($chalan_imei_number){
          ...
          $data= array(
           ...
           curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            ...
            $data['admin_main_content']=$this->load->view('Clientpage/exporter_vehicles_live_track',$data,TRUE);
            $this->load->view("Clientpage/client_master",$data);
        }else{
            $fdata['error_message']="Vehicles Device Tracker Id Not Found...";
            $this->session->set_flashdata($fdata);
            redirect('Client/ExporterChalanInfo');
        }
            }else{
    ...
    public function view_exporter_chalan_info($chalan_info_id){
        $data['chalan_data']=$this->chalanModel->all_chalan_info_by_chalan_id($chalan_info_id);
        if($data['chalan_data']){
         $data['admin_main_content']= $this->load->view('Clientpage/exporter_view_chalan',$data,TRUE);
        $this->load->view("Clientpage/client_master",$data);
        }else{
    ...
    public function exporter_all_vehicles_request(){
        $exporter_info_id=$this->session->userdata('exporter_info_id');
        $data['exporter_all_request']=$this->clientControllerModel->all_exporter_vehicles_request_by_exporter_info($exporter_info_id);
        $data['admin_main_content']=$this->load->view('Clientpage/exporter_all_vehicles_request',$data,TRUE);
	    $this->load->view("Clientpage/client_master",$data);
    }


    public function add_exporter_vehicles_request(){
        if(empty($_POST)){
            $fdata['error_message']="Direct Access Not Allowed";
                $this->session->set_flashdata($fdata);
                redirect('Client/ExporterAllVehiclesRequest');
        }else{
            ...
            $vehicles_request_data['vehicles_request_user_number']      = $this->session->userdata('exporter_primary_mobile_number');
            $vehicles_request_data['vehicles_request_some_note']        = $vehicles_request_some_note;
            $vehicles_request_data['vehicles_request_user_id']          = $this->session->userdata('exporter_info_id');
            $vehicles_request_data['vehicles_request_user_type']        = 'exporter';
            $vehicles_request_data['vehicles_request_created_date']     = date("Y-m-d H:i:s");
          
            $insert_exporter_vehicles_request= $this->clientControllerModel->save_exporter_vehicles_request($vehicles_request_data);
