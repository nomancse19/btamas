    ...
    <section class="content">
      <div class="row">
       ...
       <a href="<?php echo base_url();?>Cholotransportowner/ProfitReport"><button class="btn btn-success">Profit Report</button></a>
            ...
            <form role="form" action="<?php echo base_url();?>Cholotransportowner/GetCategoryWiseIncomeReport" enctype="multipart/form-data" method="get">
                      <input type="hidden" name="<?=ReportController::csrf_name();?>" value="<?=ReportController::csrf_token();?>" />

                          <div class="row">
                          <div class="col-sm-4">
                          <div class="form-group">
                          <label for="image_slider_link">Select Income Category</label>
                          <select class="form-control select2" required='' name="income_category_id" style="width: 100%;">
                                <option value= '' selected="selected">Select Income Category..</option>
                               <?php if($all_publish_income_category){ 
                                 foreach($all_publish_income_category as $all_publish_income_category_list){
                                 ?>
                                <option value="<?php echo $all_publish_income_category_list->income_category_id;?>"><?php echo $all_publish_income_category_list->income_category_name;?></option>
                                 <?php } ?>
                               <?php } else{?>
