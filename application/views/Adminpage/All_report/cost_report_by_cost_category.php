...
.box-contents{
  ...
  <div class="content-wrapper">
       ...
       <a href="<?php echo base_url();?>Cholotransportowner/ProfitReport"><button class="btn btn-success">Profit Report</button></a>
            ...
            <aside id="info-block">
                <section class="file-marker">
                  <div>
                    <div class="box-title">
                        Category Wise Costing Report..
                    </div>
                    <div class="box-contents">
                      <div id="audit-trail">
            
            <form role="form" action="<?php echo base_url();?>Cholotransportowner/GetCategoryWiseCostingReport" enctype="multipart/form-data" method="get">
                      <input type="hidden" name="<?=ReportController::csrf_name();?>" value="<?=ReportController::csrf_token();?>" />

                          <div class="row">
                          <div class="col-sm-4">
                          <div class="form-group">
                          <label for="image_slider_link">Select Costing Category</label>
                          <select class="form-control select2" required='' name="costing_category_id" style="width: 100%;">
                                <option value= '' selected="selected">Select Costing Category..</option>
                               <?php if($all_publish_costing_category){ 
                                 foreach($all_publish_costing_category as $all_publish_costing_category_list){
                                 ?>
                                <option value="<?php echo $all_publish_costing_category_list->cost_category_id;?>"><?php echo $all_publish_costing_category_list->cost_category_name;?></option>
                                 <?php } ?>
                               <?php } else{?>
                               <option value=''> No Available Category Found..</option>
                               <?php } ?>
                              </select>
                          </div>
            ...
            <div>
             <?php
             if(isset($all_cost_data)){
             $total_all_cost_data=count($all_cost_data);


             if($total_all_cost_data>0){
             ?>
    ...
    <button class="btn btn-warning" style="text-align:center;margin-left:0%" onclick="savepdf()">Save PDF Report</button>
            ...
            <title>BTAMS - Category Wise Costing Report..</title>
                 <p style="text-align:center;font-weight:bold;">
                  <span style="font-size:25px;">  BTAMS</span><br>
                  <span style="font-size:19px;"> <?php echo $costing_category_name; ?> Report</span><br>
              ...
              <table class="table table-bordered" style="float: left;width:100%;margin-left:2px;">
                  <thead>
                    <tr>
                    <th colspan="5" style="text-align:center;font-size:12px;">Costing Account</th>
                    </tr>
                  </thead>
                  ...
                  <tr style="font-size:11px;">
                   <th width="5%">SL</th>
                   <th width="15%">Date</th>
                   <th width="20%">Cost Category Name</th>
                   <th width="40%">Costing Details</th>
                   <th width="20%" style="text-align:right;">Cost Amount</th>
                   </tr>
                  <?php if($all_cost_data){ ?>
                    <?php 
                    $no=0;
                    $cost_details_amount=0;
                    ?>
                  <?php foreach($all_cost_data as $all_cost_data_info){ 
                    $cost_details_amount=$cost_details_amount+$all_cost_data_info->cost_details_amount;
                    $no++;
                    
                    ?>
                  <tr style="font-size:10px;">
                    <td><?php echo $no; ?></td>
                    <td><?php echo $all_cost_data_info->cost_details_added_date_time;?></td>
                    <td><?php echo $all_cost_data_info->cost_category_name;?></td>
                    <td><?php echo mb_strimwidth($all_cost_data_info->cost_details_name,0,66,"..");?></td>
                    <td style="text-align:right;"><?php echo $all_cost_data_info->cost_details_amount;?></td>
                  </tr>  
                  <?php } ?>
                    <tr style="font-size:10px;">
                      <td colspan="4" style="text-align:center;font-weight:bold;">Total</td>
                      <td style="text-align:right;font-weight:bold;"><?php echo $cost_details_amount; ?></td>
