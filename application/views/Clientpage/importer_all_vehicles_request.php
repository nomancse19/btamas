      ...
      <div class="row">
        <div class="col-12">
          <div class="card">
            ...
            <form class="form-inline" action="<?php echo base_url();?>Client/AddImporterVehiclesRequest" method="post">
            <input type="hidden" name="<?=ImporterClientController::csrf_name();?>" value="<?=ImporterClientController::csrf_token();?>" />
          ...
          <hr>         
            <table id="datatable" class="table table-bordered table-hover">
                ...
                <tbody>
                  <?php
                  if($importer_all_request){
                    $sl=0;
                    foreach($importer_all_request as $all_importer_all_request){
                      $sl++;
                  ?>
                  <tr>
                  <td><?php echo $sl;?></td>
                  <td><?php echo $all_importer_all_request->vehicles_request_created_date;?></td>
                  <td><?php echo $all_importer_all_request->vehicles_request_user_name;?></td>
                  <td><?php echo $all_importer_all_request->vehicles_request_user_number;?></td>
                  <td style="text-align:center;">
                      <?php if($all_importer_all_request->vehicles_request_status==0){?>
                      <span class="badge badge-warning">Pending</span>
                      <?php } ?>
                      <?php if($all_importer_all_request->vehicles_request_status==1){?>
                      <span class="badge badge-primary">Processing</span>
                      <?php } ?>
                      <?php if($all_importer_all_request->vehicles_request_status==2){?>
                      <span class="badge badge-info" style="font-weight:bold;">Accepted,Processing Chalan</span>
                      <?php } ?>
                      <?php if($all_importer_all_request->vehicles_request_status==3){?>
                      <span class="badge badge-success">Completed.</span>
                      <?php } ?>
                      <?php if($all_importer_all_request->vehicles_request_status==4){?>
